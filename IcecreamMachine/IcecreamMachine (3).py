...
# make a  folder under the folder you're putting these files in
...
class IceCreamMachine:
    ...
    containers = [Container(name="Waffle Cone", cost=1.5), 
                  Container(name="Sugar Cone", cost=1), 
                  Container("Cup", cost=1)]
    flavors = [Flavor(name="Vanilla", quantity=100, cost=1), 
               Flavor(name="Chocolate", quantity=100, cost=1), 
               Flavor(name="Strawberry", quantity=100, cost=1)]
    toppings = [Toppings(name="Sprinkles", quantity=200, cost=.25), 
                Toppings(name="Chocolate Chips", quantity=200, cost=.25), 
                Toppings(name="M&Ms", quantity=200, cost=.25), \
                Toppings(name="Gummy Bears", quantity=200, cost=.25), 
                Toppings(name="Peanuts", quantity=200, cost=.25)] 
    ...
    def pick_container(self, choice):
        for c in self.containers:
            if c.name.lower() == choice:
                c.use()
                IceCreamMachine.inprogress_icecream.append(c)
                return
        raise InvalidChoiceException
    print(inprogress_icecream)
    ...
    def calculate_cost(self):
        self.temp_total+=abs(sum([i.cost for i in self.inprogress_icecream]))
