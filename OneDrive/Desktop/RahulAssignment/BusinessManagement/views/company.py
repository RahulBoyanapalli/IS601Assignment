...
def search():
    ...
    try:
        ...
        flash(
            f"Following exception occured while fetching the company list:{str(e)}",
            "danger",
        )
    # hint: use allowed_columns in template to generate sort dropdown
    return render_template(
        "list_companies.html", rows=rows, allowed_columns=allowed_columns_tuples
    )
    ...
    if request.method == "POST":
        ...
        if not has_error:
            try:
                ...
                flash(
                    f" Following exception occured while adding the company: {str(e)}",
                    "danger",
                )
    ...
    if id is None:
        ...
        if request.method == "POST":
            ...
            try:
                ...
                flash(
                    f" Following exception occured while updating the company: {str(e)}",
                    "danger",
                )
        try:
            # TODO edit-11 fetch the updated data
            result = DB.selectOne(
                "SELECT name, address, city, state, country, zip, website FROM IS601_MP2_Companies WHERE id = %s",
                id,
            )
            if result.status:
                row = result.row
            ...
            flash(
                f" Following exception occured while fetching the updated company record: {str(e)}",
                "danger",
            )
    # TODO edit-13 pass the company data to the render template
    return render_template(
        "edit_company.html", form=form, state=row["state"], country=row["country"]
    )
...
def delete():
    ...
    if id:
        try:
            ...
            flash(
                f" Following exception occured while deleting the company record: {str(e)}",
                "danger",
            )
        del args["id"]
