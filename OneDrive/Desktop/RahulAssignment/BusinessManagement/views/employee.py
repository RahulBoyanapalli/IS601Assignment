...
def search():
    ...
    if not int(limit) > 0 and int(limit) <= 100:
        flash(
            "Limit entered is out of bounds. Limit should be in between 0 and 100",
            "error",
        )
    ...
    try:
        ...
        flash(
            f"Following exception occured while fetching the Employee list: {str(e)}",
            "error",
        )
    # hint: use allowed_columns in template to generate sort dropdown
    return render_template(
        "list_employees.html", rows=rows, allowed_columns=allowed_columns_tuples
    )
...
def add():
    ...
    if request.method == "POST":
        ...
        try:
            ...
            print(e)
            flash(
                f" Following exception occured while adding the employee record: {str(e)}",
                "danger",
            )
    ...
    if id is None:
        ...
        if request.method == "POST":
            ...
            try:
                ...
                flash(
                    f" Following exception occured while updating the employee: {str(e)}",
                    "danger",
                )
        try:
            ...
            result = DB.selectOne(
                "SELECT e.first_name, e.last_name, e.email, e.company_id, IF(c.name is not null, c.name,'N/A') AS company_name from IS601_MP2_Employees e LEFT JOIN IS601_MP2_Companies c ON e.company_id = c.id WHERE e.id = %s",
                id,
            )
            if result.status:
                row = result.row
            ...
            flash(
                f" Following exception occured while fetching the updated employee record: {str(e)}",
                "danger",
            )
    # TODO edit-10 pass the employee data to the render template
    return render_template(
        "edit_employee.html", form=form, company_id=row["company_id"]
    )
...
def delete():
    ...
    if id:
        try:
            ...
            flash(
                f" Following exception occured while deleting the employee record: {str(e)}",
                "danger",
            )
        del args["id"]
